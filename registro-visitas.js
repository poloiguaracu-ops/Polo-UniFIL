...
function bind(){
 ...
 document.getElementById('registroApagarTudo').addEventListener('click',async()=>{if(!registros.length)return;if(!confirm('Deseja apagar TODOS os registros de visitas? Esta ação não pode ser desfeita.'))return;try{if(servidorDisponivel)await api('DELETE',{all:true})}catch{servidorDisponivel=false}registros=[];localSet([]);resetForm();render();toast('Histórico apagado.');});
...
async function excluir(rid){const x=registros.find(v=>v.id===rid);if(!x||!confirm(`Excluir o registro de ${x.nome}?`))return;try{if(servidorDisponivel)await api('DELETE',{id:rid})}catch{servidorDisponivel=false}registros=registros.filter(v=>v.id!==rid);localSet(registros);render();toast('Registro excluído.')}

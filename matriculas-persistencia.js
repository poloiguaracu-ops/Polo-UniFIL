...
function init(){
 ...
 if(!section||!form||!btn||!msg||btn.dataset.persistenciaAtiva==='4') return; btn.dataset.persistenciaAtiva='4'; const originalText=btn.textContent;
 ...
 async function enviar(){
  ...
  const d=dados(),snap=JSON.stringify(d);let last='';try{last=sessionStorage.getItem(LAST)||''}catch(e){} if(last===snap){msgset('Esta matrícula já foi salva no dispositivo.',true);saveLocal({id:'local-'+Date.now(),nome,curso:String(d.cursoGraduacao||''),data_solicitacao:String(d.dataSolicitacao||''),criado_em:Date.now(),dados:d});return}
